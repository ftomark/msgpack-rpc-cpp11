...
#ifndef FIR_RPCSERVER_H_S0HB5KXY
...
public:
	...
	//提供自定义dispatcher进行初始化
    server(detail::default_dispatcher* disp);
	//默认构造函数，另外提供函数进行地址和端口的初始化
    server();
    ...
    virtual server& operator=(server&& other);

    virtual void accept(std::string const &address, uint16_t port);
    virtual void accept(uint16_t port);

	void set_dispatcher(detail::default_dispatcher* disp)
	{
		disp_.reset(disp);
		//disp_ = disp;
	}
	...
	//从消息队列里取出所有消息执行，执行完返回，不阻塞
	virtual void poll();
	//从消息队列里取出一条消息执行，执行完返回，不阻塞
	virtual void poll_one();
    ...
    virtual void async_run(std::size_t worker_threads = 1);
    ...
    template <typename F> void bind(std::string const &name, F func) {
        disp_->event_handle.bind(name, func);
    }
    ...
    virtual void suppress_exceptions(bool suppress);
...
protected:
    ...
    std::shared_ptr<detail::default_dispatcher> disp_;
};
