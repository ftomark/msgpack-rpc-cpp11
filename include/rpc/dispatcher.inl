...
template <typename F> void EventManager::bind(std::string const &name, F func) {
...
template <typename F>
void EventManager::bind(std::string const &name, F func,
...
void EventManager::bind(std::string const &name, F func,
    ...
    funcs_.insert(
        std::make_pair(name, [func, name](RPCLIB_MSGPACK::object const &args) {
            ...
            detail::call(func, args_real);//std::apply(func,args_real); 在这一步先取出user指针，再调用对应函数 std::apply(func,user,args_real);
            return rpc::detail::make_unique<RPCLIB_MSGPACK::object_handle>();
        }));
}
